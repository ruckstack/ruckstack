...
func Start() error {
	...
	os.MkdirAll(common.InstallDir()+"/logs", os.FileMode(0755))

	k3sLogs, err := os.OpenFile(common.InstallDir()+"/logs/k3s.log", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return err
	}
	...
	k3sArgs := []string{
		...
		"--data-dir", common.InstallDir() + "/data",
		"--kubelet-arg", "root-dir=" + common.InstallDir() + "/data/kubelet",
	...
	if localConfig.Join.Server == "" {
		k3sArgs = append(k3sArgs,
			...
			"--no-deploy", "traefik",
			"--default-local-storage-path", common.InstallDir()+"/data/local-storage",
	...
	k3sStartCommand = exec.Command(common.InstallDir()+"/lib/k3s", k3sArgs...)
	...
	if err := files.CheckFilePermissions(common.InstallDir(), "config/kubeconfig.yaml"); err != nil {
		return err
	}
