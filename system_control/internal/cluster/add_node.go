...
func AddNode() error {
	...
	tokenFileContent, err := ioutil.ReadFile(filepath.Join(common.InstallDir(), "/data/server/token"))
	if err != nil {
		return err
	}

	kubeConfigFileContent, err := ioutil.ReadFile(filepath.Join(common.InstallDir(), "/config/kubeconfig.yaml"))
	if err != nil {
		return err
	}

	token := new(common.AddNodeToken)
