...
import (
	...
	"github.com/ruckstack/ruckstack/system_control/internal/util"
	"os"
	"os/exec"
	"strconv"
	"strings"
...
func Uninstall() error {
	...
	fmt.Printf("Uninstall %s from %s? [y|n] ", packageConfig.Name, common.InstallDir())
	ui.Scan()
	if ui.Text() != "y" {
	...
	command := exec.Command("pgrep", "-f", common.InstallDir())
	...
	warn(os.RemoveAll(common.InstallDir()))
