...
func StartWebserver() error {
	...
	go func() {
		if err := http.ListenAndServeTLS(":443",
			common.InstallDir()+"/data/ssl-cert.pem",
			common.InstallDir()+"/data/ssl-key.pem",
			nil); err != nil {
			log.Printf("ERROR: %s", err)
		}
	}()
...
func serveLocalFile(res http.ResponseWriter, url string) error {
	siteDownFile, err := os.Open(common.InstallDir() + "/data/web" + url)
	if err == nil {
...
func generateKeys() error {
	sslCertFilePath := common.InstallDir() + "/data/ssl-cert.pem"
	...
	keyOut, err := os.OpenFile(common.InstallDir()+"/data/ssl-key.pem", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return err
	}
	defer keyOut.Close()
