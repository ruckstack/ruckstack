...
import (
	...
	"github.com/ruckstack/ruckstack/common/k3s"
	"gopkg.in/yaml.v2"
	"io/ioutil"
	"log"
	"os"
...
func Upgrade(upgradeFile string, targetDir string) error {
	...
	common.SetInstallDir(targetDir)
	...
	serverPidData, err := ioutil.ReadFile(filepath.Join(common.InstallDir(), "data", "server.pid"))
	if err != nil {
		return err
	}
	...
	if err := extract(common.InstallDir(), zipReader); err != nil {
		return err
	}
	...
	if os.IsNotExist(err) {
		...
		imagesDir := common.InstallDir() + "/data/agent/images"
